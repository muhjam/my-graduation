...
export default function GoogleAuth({ onAuthSuccess, onAuthError }: GoogleAuthProps) {
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      setIsLoading(false);
      if (onAuthError) {
        onAuthError('Failed to connect to Google');
      }
    }
  };
    ...
    <div className="flex items-center space-x-4">
      {isAuthenticated ? (
        ...
        <button
          ...
          disabled={isLoading}
          className="flex items-center space-x-2 px-4 py-2 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed"
        >
          {isLoading ? (
